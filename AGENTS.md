...
- **运行范围**：仅匹配 `https://linux.do/*` 和 `https://*.linux.do/*`，禁止恢复为全站注入
- **Discourse 追踪**：保留 Linux.do / IDCFlare 的既有追踪逻辑；话题链接会补发 `/pageview`，必要时 fallback 到 `/t/{topicId}.json?track_visit=true&forceLoad=true`
